...
class Mlag:
    ...
    def _evaluate_batch_size(self):
        if not isinstance(self.testdat, tf.data.Dataset):
            batch_size = None
            self.testdat =tf.data.Dataset.from_tensor_slices(self.testdat)
            #TODO CHECK BATCH SIZE IS CORRECT HERE, DO WE USE ENTIRE DATASET ?
            # SHOULD WE? CHECK THEIR CODE
        else:
            batch_size = None
        return batch_size
