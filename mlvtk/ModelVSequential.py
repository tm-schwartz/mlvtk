...
class ModelVSequential(tf.keras.Sequential):
    def __init__(self, model, msaver_path='mlvtk'):
        super(ModelVSequential, self).__init__(model.inputs, model.outputs,
    ...
    def gen_path(self):
        ...
        assert os.path.isdir(self.msave_path), 'Could not find model save path'
        files = [self.msave_path + file_name for file_name in
                 sorted(os.listddir(self.msave_path), key=lambda x:
                        int(x.split('_')[-1][:-3]))]
        ...
        def tensorlist_to_tensor_tf(weights):
            return np.concatentate([w.flatten() if w.ndim > 1 else w for w in
                                    weights])
    ...
    def plot(self, title_text, save_file):
        ...
        fig.update_layout(title=dict(text=title_text, x=.7), autosize=False,
